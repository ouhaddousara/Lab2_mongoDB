...
## Why bulk_write() for sentiment enrichment (Q5)?
Instead of 200 separate `update_one()` calls (200 network round-trips),
`bulk_write()` sends all updates in a single batch → ~50x faster.

## Why allowDiskUse=True in aggregations?
MongoDB limits in-memory aggregation to 100MB. The sample_mflix dataset
...
## Why get_db() as a centralized function?
